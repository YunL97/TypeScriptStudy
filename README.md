...
*  네임스페이스: 그냥 고유한 식별자를 생성하기 위한 일종의 접두사라고 생각하면될듯 -> 리액트 컴포넌트의 이름충돌을 방지하기 위해 사용 => 코드를 작업할 때 찾고자 하는것을 신속히 찾을수 있고 다음 작업할 곳을 찾기위해 큰파일에서 스크롤을 움직이지 않아도 되는 작고 집중된파일로인해 훨씬 관리에 용이합니다.
```
